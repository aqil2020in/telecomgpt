...
ISSUE_KEYWORDS = {
    ...
    "vonr": ("vonr", "volte", "voice", "5qi-1", "5qi 1", "ims", "mute call", "rtp"),
    "anr": ("anr", "neighbor relation", "ncr", "pci conflict", "pci collision", "missing neighbor"),
    "config_audit": ("config audit", "parameter drift", "cm audit", "golden config", "a3 offset", "prach config"),
    "gnb_syslog": ("syslog", "gnb log", "du crash", "cu crash", "ngap", "xnap", "cell outage"),
}

_EXTRA_ISSUE_TYPES = frozenset({
    "rf_coverage", "coverage", "transport", "core", "complaint",
    "vonr", "anr", "config_audit", "gnb_syslog",
})
...
def detect_issue_type(query: str, explicit: str | None = None) -> str:
    ...
    if scores[best] == 0:
        return "handover"
    if best in ("transport", "core", "complaint", "rf_coverage", "vonr", "anr", "config_audit", "gnb_syslog"):
